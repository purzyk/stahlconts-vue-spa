        ...
        <div class="px-p8">
            <div v-if="realizacje.length" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="col-span-1" v-for="realizacja in realizacje" :key="realizacja.id">
                    <router-link :to="{ name: 'RealizacjeDetails', params: { id: realizacja.id } }">
                        <div class="pb-20 ">
...
export default {
    ...
    mounted() {
        fetch('https://stahlconst.purzycki.pl/wp-json/wp/v2/realizacje?_embed=wp:term')
            .then(res => res.json())
